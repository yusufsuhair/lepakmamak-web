...
- Final network build: 100 synthetic movement clients completed the 900-second soak with all 100 connected and no unplanned closes/errors. RTT p50 28 ms / p95 109 ms / p99 215 ms (87,200 samples), approximately 4.68 MB/s payload, final RSS 105 MB. A short initial join burst exercised snapshot backpressure; steady-state final dropped-frame rate was zero. This result does not identify the cause of the earlier intermediate-run disconnects.
- 181 focused regression tests passed; six transport tests (including a subsequently added real-wire mixed-client/teleport/resync case) passed. The real SFU recovery flow also passed against the deployed dev backend.
- Main subsequently included tree release 1.40.1 (`59eff3e`); nine foliage/Mamak integration tests and the combined production build passed. Network implementation remains `dbc9446`. Production release version is therefore 1.40.1.
- Raw logs and results are retained at `/Users/yusufsuhair/Downloads/lepak-network-release-2026-09-12/`.
