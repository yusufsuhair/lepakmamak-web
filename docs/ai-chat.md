...
Run the server with `DEEPSEEK_API_KEY` set in its secret environment. The model is `deepseek-flash`; no client environment variable or SDK is needed. Without the key, automated replies are disabled.
