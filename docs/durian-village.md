...
Children and selected adults follow small open-ground walking loops. Upin and Ipin play a continuous scripted badminton rally on the central court with rackets, a net and an arcing shuttlecock. This is an ambient spectator activity, not a player-controlled match. Reduced motion freezes animation through the host's existing time-zero update; distance culling remains unchanged. `nearby` resolves dialogue against the current animated positions instead of spawn coordinates.
