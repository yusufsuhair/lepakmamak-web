...
2. The live Railway origin reported `railway/us-west2`. The Railway service region settings select one Singapore replica. The existing service used the legacy `sfo` region identifier, so that entry must explicitly be removed when moving it; `us-west=0` alone does not remove `sfo`. Verify the live `X-Railway-Upstream-Zone` header after deployment. Reproduce the region setting with `railway scale --service 9a776d22-a5d8-4151-8059-300149fce72c sfo=0 southeast-asia=1 --json`. Do not add replicas yet: rooms, chair ownership and account-session enforcement are process-local and would split across instances.
