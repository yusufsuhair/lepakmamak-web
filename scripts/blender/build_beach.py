...
def image(name,arr,data=False):
    ...
    path=OUT/'textures'/f'{name}.png';tmp.filepath_raw=str(path);tmp.file_format='PNG';tmp.save();bpy.data.images.remove(tmp)
...
def texset(kind):
    if kind not in SETS:
        c,n=getattr(TX,kind)();SETS[kind]=(image(kind+'_color',c),image(kind+'_normal',n,True))
    return SETS[kind]

def pbr(name,kind,tint,rough,tile,strength=1.0,two_sided=False,normal_scale=1.0):
...
def export(e):
    ...
    report={'asset':'LM_ENV_Beach','origin':[0,0,0],'shore':{'sandY':SAND_Y,'foreshore':[FORE0,FORE1],'nearY':NEAR_Y,'seaY':SEA_Y},
        ...
        'textures':sorted(IMAGES),'triangles':tris,'bytes':path.stat().st_size,'draws':len(e.children)}
