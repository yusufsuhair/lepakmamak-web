...
try{
 ...
 const base=process.env.TEST_BASE_URL||'https://lepakmamak.pages.dev';await page.goto(`${base}/?room=profile-smoke`);await page.getByRole('button',{name:"Jom, let's go"}).click();await page.locator('#auth-mode').click();await page.locator('#auth-email').fill(email);await page.locator('#auth-password').fill(password);await page.locator('#auth-submit').click();await expect(page.locator('#multiplayer-status-text')).toHaveText('CITY ONLINE',{timeout:20000});
 await page.locator('#menu').click();await page.locator('#open-edit-profile').click();await page.locator('#profile-bio').fill('Here for good company');await page.locator('#profile-hometown').fill('Shah Alam');await page.locator('#profile-mamakOrder').fill('Roti telur + teh tarik');await page.getByRole('button',{name:'Save profile',exact:true}).click();await expect(page.locator('#edit-profile')).not.toBeVisible({timeout:15000});
