...
export function createWall(services={}){
 ...
 async function handle(request,response){
  ...
  try{
   if(url.pathname==='/wall/posts'&&request.method==='GET'){const {data,error}=await db.from('social_posts').select('*').order('created_at',{ascending:false}).order('id',{ascending:false}).limit(LIMIT);if(error)throw error;reply(response,200,{posts:data.map(format)});return true;}
