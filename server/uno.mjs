...
export function createUno(send,now=Date.now,pick=randomInt){
 ...
 const fresh=tableId=>({id:randomUUID(),tableId,revision:0,phase:'lobby',round:0,players:[],host:null,deck:[],discard:[],color:'red',direction:1,turn:null,drawn:null,ends:0,event:null,eventId:0,unoTarget:null,winner:null,roundPoints:0,dry:0});
 ...
 function view(g,id){const me=find(g,id),top=g.discard.at(-1);return {id:g.id,tableId:g.tableId,revision:g.revision,phase:g.phase,round:g.round,host:g.host,self:id,turn:g.turn,direction:g.direction,color:g.color,top:top||null,ends:g.ends,serverTime:now(),event:g.event,winner:g.winner,roundPoints:g.roundPoints,unoTarget:g.unoTarget,drawn:me?.id===g.turn?g.drawn:null,hand:me&&!me.left?me.hand:[],players:g.players.map(p=>({id:p.id,name:p.name,count:p.hand.length,score:p.score,left:p.left,online:!p.missingAt,uno:p.uno})),playable:g.phase==='playing'&&g.turn===id&&me?me.hand.filter(c=>(!g.drawn||g.drawn===c.id)&&unoPlayable(c,top,g.color,me.hand)).map(c=>c.id):[]};}
 ...
 function win(g,id,forfeit=false){g.winner=id;g.unoTarget=null;g.ends=0;g.turn=null;const player=find(g,id);g.roundPoints=forfeit?0:g.players.filter(p=>p.id!==id&&!p.left).reduce((sum,p)=>sum+p.hand.reduce((n,c)=>n+unoPoints(c),0),0);if(player)player.score+=g.roundPoints;g.phase=player?.score>=500?'finished':'round-over';event(g,'win',player?`${player.name} menang! +${g.roundPoints} mata${forfeit?' · lawan telah keluar':''}`:'Pusingan seri. Tiada kad boleh diambil.',id);}
 function start(g){g.round++;g.deck=shuffle(unoDeck());g.discard=[];g.direction=1;g.drawn=null;g.unoTarget=null;g.winner=null;g.roundPoints=0;g.dry=0;g.players=g.players.filter(p=>!p.left);for(const p of g.players){p.hand=[];p.uno=false;}for(let i=0;i<7;i++)for(const p of g.players)draw(g,p,1);let index=g.deck.findIndex(c=>/^\d$/.test(c.value));g.discard.push(g.deck.splice(index,1)[0]);g.color=g.discard[0].color;g.turn=g.players[(g.round-1)%g.players.length].id;g.phase='dealing';g.ends=now()+3200;event(g,'deal','Mengocok dan membahagi 7 kad kepada setiap pemain.');}
 ...
 return {tick,handle(ps,p,m){if(typeof m.type!=='string'||!m.type.startsWith('uno-'))return false;const tableId=tableOf(p);if(!tableId){send(p.ws,{type:'notice',message:'Duduk di meja untuk bermain UNO.'});return true;}tick(ps);const games=map(ps);if(!games.has(tableId))games.set(tableId,fresh(tableId));let g=games.get(tableId);const id=key(p),me=find(g,id);const reply=()=>send(p.ws,{type:'uno-state',game:view(g,id)});
  ...
  if(m.type==='uno-start'&&g.host===id&&['lobby','round-over'].includes(g.phase)&&active(g).length>=2){start(g);publish(ps,g);return true;}
