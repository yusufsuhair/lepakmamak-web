...
export function createUno(send,now=Date.now,pick=randomInt){
 ...
 function start(g){if(g.phase==='finished'){g.round=0;for(const p of g.players)p.score=0;}g.round++;g.deck=shuffle(unoDeck());g.discard=[];g.direction=1;g.drawn=null;g.unoTarget=null;g.winner=null;g.roundPoints=0;g.dry=0;g.players=g.players.filter(p=>!p.left);g.breakdown=null;g.target=matchTarget(g.players.length);for(const p of g.players){p.hand=[];p.uno=false;}for(let i=0;i<7;i++)for(const p of g.players)draw(g,p,1);let index=g.deck.findIndex(c=>/^\d$/.test(c.value));g.discard.push(g.deck.splice(index,1)[0]);g.color=g.discard[0].color;g.turn=g.players[(g.round-1)%g.players.length].id;g.phase='dealing';g.ends=now()+3200;event(g,'deal','Shuffling and dealing 7 cards to each player.');}
 function take(g,p){g.unoTarget=null;const count=draw(g,p,1);g.dry=count?0:g.dry+1;if(g.dry>=active(g).length){win(g,null);return;}if(count&&unoPlayable(p.hand.at(-1),g.discard.at(-1),g.color,p.hand)){g.drawn=p.hand.at(-1).id;g.ends=now()+25000;}else turn(g,nextId(g,p.id));event(g,'draw',`${p.name} draws ${count} cards.`,p.id,count);}
 function depart(g,p){const next=g.turn===p.id?nextId(g,p.id):g.turn;p.left=true;p.missingAt=0;g.deck.push(...p.hand);shuffle(g.deck);p.hand=[];if(g.unoTarget===p.id)g.unoTarget=null;if(active(g).length<2){win(g,active(g)[0]?.id||null,true);}else{if(g.turn===p.id)turn(g,next);event(g,'leave',`${p.name} left the round.`,p.id);}}
 function tick(ps){const games=rooms.get(ps);if(!games)return;for(const g of games.values()){const present=new Set(members(ps,g.tableId).map(key));let changed=false;for(const p of [...g.players]){if(present.has(p.id)){if(p.missingAt){p.missingAt=0;changed=true;}}else if(['lobby','round-over','finished'].includes(g.phase)){g.players=g.players.filter(q=>q!==p);changed=true;}else if(!p.left){if(!p.missingAt){p.missingAt=now();changed=true;}if(now()-p.missingAt>=60000){depart(g,p);changed=true;}}}if(!g.players.some(p=>p.id===g.host&&!p.left&&present.has(p.id))){const host=g.players.find(p=>!p.left&&present.has(p.id))?.id||null;if(host!==g.host){g.host=host;changed=true;}}if(g.phase==='dealing'&&now()>=g.ends){g.phase='playing';turn(g,g.turn);event(g,'turn','Cards are ready. Let’s play!',g.turn);changed=true;}else if(g.phase==='playing'&&now()>=g.ends){const p=find(g,g.turn);if(p){if(g.drawn){g.unoTarget=null;turn(g,nextId(g,p.id));event(g,'pass',`${p.name}: time ran out; next turn.`,p.id);}else{take(g,p);if(g.drawn)turn(g,nextId(g,p.id));}changed=true;}}if(changed){g.revision++;publish(ps,g);}if(!g.players.length&&!members(ps,g.tableId).length)games.delete(g.tableId);}}
 return {tick,canRematch(ps,p){return rooms.get(ps)?.get(tableOf(p))?.phase==='finished';},handle(ps,p,m){if(typeof m.type!=='string'||!m.type.startsWith('uno-'))return false;const tableId=tableOf(p);if(!tableId){send(p.ws,{type:'notice',message:'Sit at a table to play UNO.'});return true;}tick(ps);const games=map(ps);if(!games.has(tableId))games.set(tableId,fresh(tableId));let g=games.get(tableId);const id=key(p),me=find(g,id);const reply=()=>send(p.ws,{type:'uno-state',game:view(g,id)});
  if(m.type==='uno-open'){reply();return true;}
  if(m.type==='uno-join'&&['lobby','round-over','finished'].includes(g.phase)){if(!me&&g.players.length<4){g.players.push({id,name:p.name,hand:[],score:0,uno:false,left:false,missingAt:0});g.host??=id;event(g,'join',`${p.name} is ready.`);publish(ps,g);}else if(me?.left&&active(g).length<4){me.left=false;me.missingAt=0;event(g,'join',`${p.name} is ready.`);publish(ps,g);}else reply();return true;}
  ...
  if(m.type==='uno-catch'&&g.unoTarget&&g.unoTarget!==id){const target=find(g,g.unoTarget);draw(g,target,2);g.unoTarget=null;event(g,'catch',`${target.name} forgot UNO! +2 cards.`,target.id,2);publish(ps,g);return true;}
  if(g.turn!==id){reply();return true;}
  if(m.type==='uno-draw'&&!g.drawn){take(g,me);publish(ps,g);return true;}
  if(m.type==='uno-pass'&&g.drawn){g.unoTarget=null;turn(g,nextId(g,id));event(g,'pass',`${me.name} keeps the card.`,id);publish(ps,g);return true;}
  if(m.type==='uno-play'){const card=me.hand.find(c=>c.id===m.cardId);if(!card||g.drawn&&card.id!==g.drawn||!unoPlayable(card,g.discard.at(-1),g.color,me.hand)||card.color==='wild'&&!UNO_COLORS.includes(m.color)){reply();return true;}g.unoTarget=null;g.dry=0;me.hand=me.hand.filter(c=>c!==card);g.discard.push(card);g.color=card.color==='wild'?m.color:card.color;me.uno=me.hand.length===1&&m.uno===true;if(me.hand.length===1&&!me.uno)g.unoTarget=id;let next=nextId(g,id);if(card.value==='reverse'){g.direction*=-1;next=active(g).length===2?id:nextId(g,id);}else if(card.value==='skip')next=nextId(g,id,2);else if(['draw2','draw4'].includes(card.value)){const target=find(g,next);draw(g,target,card.value==='draw2'?2:4);next=nextId(g,id,2);}event(g,me.uno?'uno':'play',`${me.name} plays ${card.value}${me.uno?' · UNO!':''}`,id);if(!me.hand.length)win(g,id);else turn(g,next);publish(ps,g);return true;}reply();return true;}};
}
