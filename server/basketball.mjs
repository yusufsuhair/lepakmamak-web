...
export function createBasketball(send,now=Date.now){
 ...
 function tick(ps){let g=get(ps);const people=[...ps.values()].filter(inBasketball);
  ...
  if(g.shot){const s=g.shot,t=Math.min(1.6,(now()-s.at)/1000),u=Math.min(1,t/1.15);g.ball={x:s.x+(s.tx-s.x)*u,z:s.z+(s.tz-s.z)*u,y:t<=1.15?1.65+(3.05-1.65)*u+3.6*Math.sin(Math.PI*u):Math.max(.25,3.05-(t-1.15)*7)};
   if(t>=1.15&&!s.scored){s.scored=true;if(s.good){const entry=g.scores[s.id]||(g.scores[s.id]={id:s.id,name:s.name,points:0});entry.points+=s.points;g.notice=`${s.name} masuk! +${s.points} mata`; }else g.notice='Tak masuk! Dekati bola untuk rebound.';}
