...
export function createTableSocial(send){
 ...
 return {sync(players,force=false){
  const tables=locations.map(t=>({id:t.id,name:t.name,capacity:chairs.filter(c=>c.tableId===t.id).length,occupants:[...players.values()].filter(p=>tableForChair.get(p.chairId)===t.id).map(p=>({id:p.id,name:p.name,chairId:p.chairId}))}));
