...
export function createLukis(send,now=Date.now,pick=randomInt){
 ...
 function next(g){
  if(g.round>=g.order.length*g.rounds){g.phase='finished';g.ends=0;return;}
  g.round++;g.drawer=g.order[(g.round-1)%g.order.length];g.history=[];g.cursor=0;g.lines=[];g.solved=[];g.guesses=[];g.roundWinners=[];g.roundPoints={};g.boardVersion=0;g.lastBatch=0;g.maxStroke=0;g.totalPoints=0;g.lastHint=0;g.guessAt={};
