...
export function createWerewolf(send,now=Date.now,pick=randomInt){
 ...
 const fresh=()=>({id:randomUUID(),phase:'lobby',revision:0,day:0,ends:0,size:7,host:null,players:[],actions:{},votes:{},judgments:{},accused:null,accusers:[],log:[],chat:[],winner:null});
 function room(ps){if(!rooms.has(ps))rooms.set(ps,fresh());return rooms.get(ps);}
 const alive=g=>g.players.filter(p=>p.alive);
 const entry=(g,id)=>g.players.find(p=>p.id===id);
 function log(g,text){g.log.push(text);g.log=g.log.slice(-30);}
 function phase(g,value){g.phase=value;g.revision++;g.ends=now()+(WEREWOLF_TIMES[value]||0);g.actions={};if(value==='night'){g.day++;g.votes={};g.judgments={};g.accused=null;g.accusers=[];}if(value==='vote')g.votes={};if(value==='judgment')g.judgments={};}
 function finish(g){const living=alive(g),wolves=living.filter(p=>evil(p.role));if(!wolves.length||wolves.length>=living.length-wolves.length){g.winner=!wolves.length?'good':'evil';phase(g,'finished');log(g,g.winner==='good'?'The villagers win! Every wolf has been eliminated.':'The wolves win! The village has been overrun.');return true;}return false;}
 function eliminate(g,p,reason){if(!p?.alive)return;p.alive=false;const reasons={malam:'night attack',undian:'vote',keluar:'leaving the game','meninggalkan permainan':'leaving the game'};log(g,`${p.name} was eliminated (${reasons[reason]||reason}). Role: ${p.role}.`);}
 // The vote is public the moment it is cast. Twenty-five blind seconds while everyone
 // shouts names over voice is the pacing gap, not a secret worth keeping.
 // Headcount, never the Mayor-weighted total advance() uses: a doubled vote names the Mayor.
 function tallyOf(g){
  if(g.phase==='vote')return alive(g).reduce((t,p)=>{const v=g.votes[p.id];if(v)t[v]=(t[v]||0)+1;return t;},{});
  if(g.phase==='judgment')return g.accusers.filter(i=>entry(g,i)?.alive&&i!==g.accused).reduce((t,i)=>{const v=g.judgments[i];if(v)t[v]=(t[v]||0)+1;return t;},{kill:0,spare:0});
  return null;
 }
 function view(g,id){const me=entry(g,id),isWolf=me?.alive&&evil(me.role),finished=g.phase==='finished';return{id:g.id,phase:g.phase,revision:g.revision,day:g.day,ends:g.ends,serverTime:now(),size:g.size,sizes:g.phase==='lobby'?WEREWOLF_SIZES:null,host:g.host,self:id,winner:g.winner,accused:g.accused,players:g.players.map(p=>({id:p.id,name:p.name,alive:p.alive,online:!p.missingAt,role:finished||p.id===id||!p.alive||p.revealed||(isWolf&&evil(p.role))?p.role:null})),role:me?.role||null,inspection:me?.inspection||null,inspections:me?.inspections||[],tally:tallyOf(g),selected:g.phase==='night'?g.actions[id]||null:g.phase==='vote'?g.votes[id]||null:g.judgments[id]||null,canJudge:g.accusers.includes(id)&&id!==g.accused&&!!me?.alive,log:g.log,chat:g.chat.filter(m=>m.channel==='day'||finished||m.channel==='wolves'&&isWolf||m.channel==='ghosts'&&me&&!me.alive).slice(-60)};}
 function publish(ps,g){for(const p of ps.values())if(seated(p))send(p.ws,{type:'werewolf-state',game:view(g,key(p))});}
 function resolveNight(g){const living=alive(g),wolves=living.filter(p=>evil(p.role)),choices=wolves.map(p=>g.actions[p.id]).filter(id=>living.some(p=>p.id===id&&!evil(p.role)));let victimId=choices.length?choices.every(id=>id===choices[0])?choices[0]:choices[pick(choices.length)]:null;const doctor=living.find(p=>p.role==='doctor'),seer=living.find(p=>p.role==='seer');if(seer){const target=entry(g,g.actions[seer.id]);if(target?.alive){seer.inspection={name:target.name,team:target.role==='werewolf'?'evil':'good',day:g.day};seer.inspections=[...seer.inspections||[],seer.inspection].slice(-8);}}
  const victim=entry(g,victimId);if(victim&&doctor&&g.actions[doctor.id]===victim.id){log(g,'The night passes. Nobody was killed.');}else if(victim?.role==='knight'&&!victim.used){victim.used=true;log(g,'The night passes. Nobody was killed.');}else if(victim){eliminate(g,victim,'malam');if(victim.role==='hunter'&&pick(4)<3){const targets=alive(g).filter(p=>evil(p.role));if(targets.length)eliminate(g,targets[pick(targets.length)],'Hunter');}}else log(g,'The night passes. Nobody was killed.');if(!finish(g))phase(g,'discussion');
 }
 function advance(g){if(g.phase==='night')resolveNight(g);else if(g.phase==='discussion')phase(g,'vote');else if(g.phase==='vote'){const tally={};for(const p of alive(g)){const target=g.votes[p.id];if(target&&target!=='skip'&&entry(g,target)?.alive)tally[target]=(tally[target]||0)+(p.role==='mayor'?2:1);}const ranked=Object.entries(tally).sort((a,b)=>b[1]-a[1]);if(!ranked.length||ranked[1]?.[1]===ranked[0][1]){log(g,'The vote is tied or there is no accusation. Nobody is punished today.');phase(g,'night');}else{g.accused=ranked[0][0];g.accusers=alive(g).filter(p=>g.votes[p.id]===g.accused).map(p=>p.id);log(g,`${entry(g,g.accused).name} is accused. Time to defend yourself.`);phase(g,'defense');}}else if(g.phase==='defense')phase(g,'judgment');else if(g.phase==='judgment'){const judges=g.accusers.filter(id=>entry(g,id)?.alive&&id!==g.accused),kill=judges.filter(id=>g.judgments[id]==='kill').length;const accused=entry(g,g.accused);if(accused?.alive&&kill>judges.length/2){if(accused.role==='princess'&&!accused.used){accused.used=true;accused.revealed=true;log(g,`${accused.name} is the Princess and survives once.`);}else eliminate(g,accused,'undian');}else log(g,`${accused?.name||'The accused'} is spared.`);if(!finish(g))phase(g,'night');}}
 ...
 function rematch(ps,p){
  const g=rooms.get(ps);
  // The table shell owns the rematch button, so it needs a small public bridge into the
  // private game state. A finished village must be replaced before the next lobby starts;
  // otherwise werewolf-start quite correctly refuses the old `finished` game and the old
  // roles stay on screen forever.
  if(g?.phase!=='finished')return true;
  const id=key(p);
  if(g.host!==id&&presentHost(ps,g.host)){
   send(p.ws,{type:'notice',message:'Only the host can start a new match.'});
   return false;
  }
  const next=fresh();rooms.set(ps,next);publish(ps,next);return true;
 }
 ...
 return {tick,live,silenced,canRematch(ps){return rooms.get(ps)?.phase==='finished';},rematch,handle(ps,p,m){if(typeof m.type!=='string'||!m.type.startsWith('werewolf-'))return false;if(!seated(p)){send(p.ws,{type:'notice',message:'Sit at any table to play Werewolf.'});return true;}tick(ps);let g=room(ps);const id=key(p),me=entry(g,id);const reply=()=>send(p.ws,{type:'werewolf-state',game:view(g,id)});
  ...
  if(m.type==='werewolf-rematch'&&g.phase==='finished'){rematch(ps,p);return true;}
  if(m.type==='werewolf-start'&&g.phase==='lobby'&&g.host===id&&g.players.length===g.size){let roles=werewolfRoles(g.size,pick);for(let i=roles.length-1;i>0;i--){const j=pick(i+1);[roles[i],roles[j]]=[roles[j],roles[i]];}g.players.forEach((q,i)=>q.role=roles[i]);log(g,'Roles have been dealt. Keep your role secret.');phase(g,'night');publish(ps,g);return true;}
