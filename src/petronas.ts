...
export const PETRONAS={x:-31,z:112,url:cdnUrl('assets/models/environment/LM_ENV_Petronas.glb')};
export interface PetronasSite {
  group:THREE.Group; fallback:THREE.Group;
  /** Starts the download on the first call; until it resolves the fallback and its collisions serve. */
  load():Promise<void>;
  status:{state:'idle'|'loading'|'ready'|'fallback';batches:number;triangles:number;textures:number;logoLoaded:boolean};
}
...
export function loadPetronas(scene:THREE.Scene,fallback:THREE.Group):PetronasSite {
  ...
  const status:PetronasSite['status']={state:'idle',batches:0,triangles:0,textures:0,logoLoaded:false};
  let ready:Promise<void>|undefined;
  const load=()=>{if(ready)return ready;status.state='loading';return ready=new GLTFLoader().setMeshoptDecoder(MeshoptDecoder).loadAsync(PETRONAS.url).then(gltf=>{
...
export async function preparePetronasEnvironment(renderer:THREE.WebGLRenderer,site:PetronasSite){
  await site.load();if(site.status.state!=='ready')return;
