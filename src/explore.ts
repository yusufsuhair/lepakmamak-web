...
export function createExplore(options:{releaseInput:()=>void;canOpen:()=>boolean;visit:(id:string)=>string|null;map:()=>void;enableSound:()=>void}) {
  ...
  dialog.innerHTML=`<header><div><small>YOUR CITY, A LITTLE CLOSER</small><h2 id="explore-title">Jom jalan.</h2><p id="explore-subtitle">Find your next favourite spot.</p></div><button type="button" id="close-explore" aria-label="Close Explore">×</button></header>
    ...
    <section id="explore-tutorial" class="explore-tutorial" role="tabpanel" data-explore-panel="tutorial" hidden><div class="tutorial-intro"><div><small>QUICK START · BUILT FOR THE CITY</small><h3>Know the little moves.</h3></div><p>Five useful things you can do right now. We will keep adding guides as the city grows.</p></div><div class="tutorial-grid">${EXPLORE_TUTORIALS.map(t=>`<article class="tutorial-card" data-tutorial="${t.id}"><div class="tutorial-card-top"><span>${t.number}</span><strong aria-hidden="true">${t.signal}</strong></div><figure class="tutorial-shot"><img src="${t.image}" alt="${t.alt}" width="1280" height="800" loading="lazy"></figure><h3>${t.title}</h3><p>${t.description}</p><ol>${t.steps.map(step=>`<li>${step}</li>`).join('')}</ol>${'locations' in t?`<div class="tutorial-locations">${t.locations.map(location=>`<span>${location}</span>`).join('')}</div>`:''}</article>`).join('')}</div><div class="tutorial-more"><span aria-hidden="true">＋</span><div><strong>More guides coming later.</strong><p>New activities will join this list as LepakMamak grows.</p></div></div></section>`;
