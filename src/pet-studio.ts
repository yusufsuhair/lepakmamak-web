...
type PetStudioOptions = {release?: () => void; openShop?: () => void; onName?: (name: string) => void | Promise<void>};
...
export function setupPetStudio(api: PetStudioApi, options: PetStudioOptions = {}) {
  ...
  let state: ShopState = {items: [], balance: 0}, selectedCat: PetKind, selectedRibbon = '', petName = cleanName(session?.user.user_metadata?.pet_name || ''), busy = false;
  ...
  function draw() {
    ...
    selectedCat = activeCat || 'pet-ginger'; selectedRibbon = currentRibbon();
    ...
    preview.setPet(selectedCat, selectedRibbon); previewName.textContent = petName || (catsOwned.find(item => item.id === selectedCat)?.name || 'Pet'); nameInput.value = petName;
  }
