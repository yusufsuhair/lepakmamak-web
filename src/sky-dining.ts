...
import {batchShopFallback,box,createPerson,material} from './world';
...
export function createSkyDining(scene:THREE.Scene){
 ...
 // canvas labels stay on root so the basin, the ripples and the wording survive the swap.
 ...
 const roof=box(root,0,5.4,9,36,.25,14,new THREE.MeshStandardMaterial({color:'#23263e',transparent:true,opacity:1}));box(venue,0,2.6,15.5,36,5.2,.15,'#182338');
 ...
 box(venue,0,.15,1.57,11.8,.15,.08,gold);label('WET DECK · SKY DINING',0,3.7,14.95,14);
 ...
 for(const table of rooftopTables){const x=table.x-SKY.x,z=table.z-SKY.z;
  ...
  if(large){const top=new THREE.Mesh(new THREE.CylinderGeometry(1.9,1.9,.15,32),material('#34314b'));top.position.set(x,.9,z);venue.add(top);label('WEREWOLF · 9 SEATS',x,2.6,z,4);}
 ...
 const dj=createPerson('#24304c');dj.group.position.set(-14,0,6.5);root.add(dj.group);box(venue,-14,1,5.5,3.3,.28,1.2,'#171827');obstacle(-14,5.5,3.3,1.2);
 ...
 void new GLTFLoader().loadAsync('/assets/models/environment/LM_ENV_SkyDining.glb?v=skydining-v1').then(gltf=>{
