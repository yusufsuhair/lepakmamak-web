...
export function setupCityDirectory(root:HTMLElement,canvas:HTMLCanvasElement,select:(id:string)=>void){
 ...
 canvas.addEventListener('click',event=>{if(canvas.dataset.mode==='3d'||canvas.dataset.gesture)return;const rect=canvas.getBoundingClientRect(),scale=Number(canvas.dataset.worldScale)||(canvas.width/340),centerX=Number(canvas.dataset.centerX)||0,centerZ=Number(canvas.dataset.centerZ)||0,x=((event.clientX-rect.left)/rect.width*canvas.width-canvas.width/2)/scale+centerX,y=((event.clientY-rect.top)/rect.height*canvas.height-canvas.height/2)/scale+centerZ;const label=labels.find(b=>x>=b.x&&x<=b.x+b.w&&y>=b.y&&y<=b.y+b.h);const place=label?.place||places.find(p=>Math.hypot(x-p.x,y-p.z)<7);if(place)select(place.id);});
