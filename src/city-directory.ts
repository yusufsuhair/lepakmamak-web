...
const categories:Record<string,string>={kedai:'Shops',lepak:'Hangouts',gerai:'Food stalls',minyak:'Petrol station',ibadah:'Place of worship',sukan:'Sports'};
...
export function setupCityDirectory(root:HTMLElement,canvas:HTMLCanvasElement,select:(id:string)=>void){
 ...
 canvas.addEventListener('click',event=>{const rect=canvas.getBoundingClientRect(),x=(event.clientX-rect.left)/rect.width*340-170,y=(event.clientY-rect.top)/rect.height*340-170;const label=labels.find(b=>x>=b.x&&x<=b.x+b.w&&y>=b.y&&y<=b.y+b.h);const place=label?.place||places.find(p=>Math.hypot(x-p.x,y-p.z)<7);if(place)select(place.id);});
