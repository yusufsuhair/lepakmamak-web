...
export function setupCityDirectory(root:HTMLElement,canvas:HTMLCanvasElement,select:(id:string)=>void){
 ...
 canvas.addEventListener('click',event=>{if(canvas.dataset.mode==='3d')return;const rect=canvas.getBoundingClientRect(),scale=Math.min(canvas.width/730,canvas.height/520),x=((event.clientX-rect.left)/rect.width*canvas.width-canvas.width/2)/scale-180,y=((event.clientY-rect.top)/rect.height*canvas.height-canvas.height/2)/scale-50;const label=labels.find(b=>x>=b.x&&x<=b.x+b.w&&y>=b.y&&y<=b.y+b.h);const place=label?.place||places.find(p=>Math.hypot(x-p.x,y-p.z)<7);if(place)select(place.id);});
