...
// Legoland has its own park map; keep it out of the city directory and city map pins.
export const CITY_PLACES=places.filter(place=>place.id!=='legoland');
...
export function mapLabels(ctx:CanvasRenderingContext2D){
 ...
 for(const place of CITY_PLACES){const words=(`${place.id} · ${place.name}`).split(' '),lines:string[]=[''];for(const word of words){const i=lines.length-1;if(ctx.measureText(`${lines[i]} ${word}`).width>53&&lines[i])lines.push(word);else lines[i]+=(lines[i]?' ':'')+word;}
