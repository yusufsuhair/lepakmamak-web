...
void loadMamakRealism(scene,renderer).then(async asset=>{detail=asset;lighting=configureMamakLighting(asset);lighting.setNight(night);document.querySelector('#status')!.textContent=mamakRealismStatus.state==='ready'?'Dining + V7 · Blender + PBR':'Menggunakan model asal';baseline=await loadWebAsset('/assets/models/environment/LM_ENV_MamakMaju.glb?v=mamak-v7',scene,new THREE.Vector3(-29,0,30),'Mamak V7 comparison');oldLighting=configureMamakLighting(baseline);oldLighting.setNight(night);setBefore(before);});
