...
export function setupTableSocial(send:(message:object)=>boolean,_room:string,releaseInput:()=>void){
 ...
 const lukis=setupLukis(gameSend),poker=setupPoker(gameSend);dialog.querySelector('.table-game-stage')!.append(lukis.root,poker.root);
 function selectGame(value:string){lukis.root.hidden=value!=='lukis';poker.root.hidden=value!=='poker';(dialog.querySelector('.table-game-menu') as HTMLElement).hidden=!!value;(dialog.querySelector('.table-back') as HTMLElement).hidden=!value;dialog.classList.toggle('playing-drawing',value==='lukis');if(value)send({type:value==='lukis'?'lukis-open':'poker-open'});}
 selectGame('');
 ...
 function render(){const seated=own(),id=seated?.id||selected,name=locations.find(t=>t.id===id)?.name||'Meja';
  ...
  const next=seated?.id||'';if(current!==next){current=next;selectGame('');lukis.state(null,selfId);poker.state(null,selfId);if(next)send({type:'lukis-open'});}
 ...
 return {open(tableId?:string){selected=tableId||own()?.id||selected;releaseInput();render();if(!dialog.open)dialog.showModal();dialog.querySelector<HTMLButtonElement>('#close-table-social')!.focus();},close,
  ...
  game(value:any){if(!value||value.tableId===own()?.id)lukis.state(value,selfId);},gameFeedback(kind:string,message:string){lukis.feedback(kind,message);},gameCorrect(name:string,points:number){lukis.correct(name,points);},gameLine(value:any){if(own())lukis.line(value);},poker(value:any){if(!value||value.tableId===own()?.id)poker.state(value,selfId);},
