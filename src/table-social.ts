...
export function setupTableSocial(send:(message:object)=>boolean,_room:string,releaseInput:()=>void){
 ...
 const lukis=setupLukis(gameSend),poker=setupPoker(gameSend),werewolf=setupWerewolf(gameSend),uno=setupUno(gameSend);dialog.querySelector('.table-game-stage')!.append(lukis.root,poker.root,werewolf.root,uno.root);
 function selectGame(value:string){lukis.root.hidden=value!=='lukis';poker.root.hidden=value!=='poker';werewolf.root.hidden=value!=='werewolf';uno.root.hidden=value!=='uno';(dialog.querySelector('.table-game-menu') as HTMLElement).hidden=!!value;(dialog.querySelector('.table-back') as HTMLElement).hidden=!value;dialog.classList.toggle('playing-drawing',value==='lukis');dialog.classList.toggle('playing-uno',value==='uno');if(value)send({type:value==='lukis'?'lukis-open':value==='werewolf'?'werewolf-open':value==='uno'?'uno-open':'poker-open'});}
 selectGame('');
 ...
 function render(){const seated=own(),id=seated?.id||selected,name=locations.find(t=>t.id===id)?.name||'Meja';
  ...
  const next=seated?.id||'';if(current!==next){current=next;selectGame('');lukis.state(null,selfId);poker.state(null,selfId);werewolf.state(null);uno.state(null);if(next)send({type:'lukis-open'});}
