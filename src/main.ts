...
import {setupCityDirectory,drawPlaceLabels,CITY_PLACES} from './city-directory';
...
async function init() {
  ...
  const contactShadows = createContactShadows(scene, world.shoplots.lots, world.chairs, tableLocations);
  ...
  function renderPartyRoster() {
    ...
    for (const member of visible) {
      ...
      copy.append(name, meta); row.append(face, copy);
      if (member.leader) {
        const crown = document.createElement('span'); crown.className = 'party-leader-crown'; crown.textContent = '♛'; crown.title = 'Party leader'; crown.setAttribute('aria-label', 'Party leader');
        row.append(crown);
      }
      partyRosterList.append(row);
    }
  ...
  function renderOnlinePlayers() {
    ...
    $('online-players-count').textContent = `${people.length} player${people.length === 1 ? '' : 's'} online`;
  ...
  function openPlayerOptions(x: number, y: number, proximityId = '') {
    ...
    // You cannot invite yourself, someone already in your Party, or nobody in particular.
    const relation = friendsUI.relationship(targetId);
    ...
    $('kick-party').hidden=mine||!targetId||!networkConnected||!partyMembers.has(targetId)||partyLeaderId!==networkPlayerId;
  ...
  $('kick-party').onclick = () => { closeOptions(); if (networkSocket?.readyState === WebSocket.OPEN) networkSocket.send(JSON.stringify({type:'party-kick',id:selectedProfileId})); };
  ...
  const selectMapPlace=(id:string)=>{carFinder.deselect();selectedMapPlace=id;teleportButton.disabled=teleportPending;teleportButton.textContent=`Teleport to ${CITY_PLACES.find(p=>p.id===id)?.name||'destination'}`;mapDirectory.selected(id);drawMap(true);};
  ...
  function drawMap(expanded = false) {
    ...
    const selectedPlace=CITY_PLACES.find(p=>p.id===selectedMapPlace);
  ...
  function updateHud() {
    ...
    $('map-area').textContent = area.toUpperCase();
