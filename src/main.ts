...
$('app').innerHTML = `
  ...
  <section id="hud" aria-label="Game information" hidden>
    ...
    <div id="minimap-wrap"><button type="button" id="open-map" class="map-frame" aria-label="Open city map" aria-haspopup="dialog"><canvas id="minimap" width="364" height="332" aria-label="Map showing your location and online players"></canvas><span class="map-north">N ↑ · M</span><span id="map-online" class="map-online" aria-live="polite">0 online</span></button><div class="map-caption"><span id="map-area">KAMPUNG MAJU</span><span>● YOU</span></div></div>
...
async function init() {
  ...
  function drawMap(expanded = false) {
    ...
    syncMapScope(inPark);
    const onlineCount=networkConnected?roomPlayers.length:multiplayerEndpoint?0:1;
    $('map-online').textContent=`${onlineCount} online`;
    if(inPark){
      const map=$<HTMLCanvasElement>(expanded?'expanded-map':'minimap');
      map.dataset.onlineCount=String(onlineCount);
    ...
    map.dataset.peerCount=String(cityPeers.length);map.dataset.onlineCount=String(onlineCount);
