...
export function setupGeng(endpoint: string, onState: (state: GengState | null) => void, release: () => void = () => {}, onLiveAction: (action: 'leave') => void = () => {}, onTopup: () => void = () => {}, onProfile: (id: string, name: string) => void = () => {}, onEvent: (event: GengEvent, unread: number) => void = () => {}) {
  ...
  dialog.innerHTML = `<header><div><small>LEPAKMAMAK · SOCIAL</small><h2 id="game-geng-title">Geng</h2><p>Build your squad, one request at a time.</p></div><button type="button" aria-label="Close Geng">×</button></header>
    <div class="geng-balance"><span>YOUR LEPAK COIN</span><strong id="geng-balance">—</strong></div>
    ...
    <section id="geng-create" hidden aria-labelledby="geng-create-title"><small>START A NEW SQUAD</small><h3 id="geng-create-title">Create a Geng</h3><p>It costs 1,000 Lepak Coin. You become the leader and approve every member.</p><div id="geng-topup-prompt" class="geng-topup-prompt" hidden><div><small>NOT ENOUGH LEPAK COIN</small><strong>Add Lepak Coin to continue</strong><p id="geng-topup-copy"></p></div><button type="button" id="geng-topup" class="primary">Add Lepak Coin</button></div><form id="geng-create-form"><label for="geng-create-name">Geng name<input id="geng-create-name" minlength="2" maxlength="24" autocomplete="off" required placeholder="e.g. Budak Mamak"></label><button type="submit" class="primary" id="geng-create-submit">Create Geng · 1,000 Lepak Coin</button></form></section>
  ...
  function render() {
    ...
    balance.textContent = state ? `${Number(state.balance || 0).toLocaleString('en-MY')} Lepak Coin` : '—';
    if (loading) {
    ...
    if (insufficient) el('geng-topup-copy').textContent = `You have ${Number(state.balance || 0).toLocaleString('en-MY')} Lepak Coin. You need 1,000 Lepak Coin to create a Geng.`;
