...
export function klSunAltitude(now = new Date()) {
  ...
  const sine=Math.sin(3.16 * Math.PI / 180) * Math.sin(declination) + Math.cos(3.16 * Math.PI / 180) * Math.cos(declination) * Math.cos((hour - 12) * Math.PI / 12);
  return Math.asin(THREE.MathUtils.clamp(sine,-1,1))*180/Math.PI;
}
