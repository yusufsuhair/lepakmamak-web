...
test('actual game loads Rembayung and the avatar walks from the entrance to the mezzanine',async({page},info)=>{
  ...
  const walk=async(key:string,axis:'x'|'z',target:number,direction:'less'|'greater')=>{
    ...
    try{await expect.poll(()=>page.evaluate(({axis,target,direction})=>{const p=(window as any).__lepak.position[axis];return direction==='less'?p<target:p>target;},{axis,target,direction}),{timeout:18000,intervals:[40]}).toBe(true);}
