...
test('self dance menu triggers a synchronized ten second dance and cannot be spammed',async({page})=>{
 ...
 try{await expect.poll(async()=>{try{return(await fetch('http://127.0.0.1:8088/health')).ok&&(await fetch('http://127.0.0.1:5180')).ok;}catch{return false;}}).toBe(true);
 ...
 const point=await page.evaluate(()=>(window as any).__lepak.profileScreen);await page.mouse.click(point.x,point.y,{button:'right'});await expect(page.locator('#dance-action')).toBeVisible();await page.locator('#dance-action').click();await expect.poll(()=>players.find(p=>p.name==='Dancer')?.danceUntil).toBeGreaterThan(Date.now());
...
test('dance audio falls off with distance',async({page})=>{await page.goto('/');const gains=await page.evaluate(async()=>{const {danceVolume}=await import('/src/dance.ts');return [0,3,10,18,50].map(danceVolume);});expect(gains[0]).toBe(.65);expect(gains[2]).toBeLessThan(gains[1]);expect(gains[3]).toBe(0);expect(gains[4]).toBe(0);});
