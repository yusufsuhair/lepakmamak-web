...
test('Watsons loop plays within earshot of the storefront and obeys city sounds',async({page})=>{
  // Spawn is 91 m from watsonsSpot (-56,-31), where the jingle is silent and the loop is meant
  // to stay paused. Arrive at the storefront instead: the loop plays what the player can hear.
  const moveTo=await enterAt(page,-56,-25);
