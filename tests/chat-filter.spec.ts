...
test('server sends only the censored message to sender and peers', async () => {
 ...
 const clients:WebSocket[]=[], received:string[][]=[[],[]];
 try {
  ...
  for(let i=0;i<2;i++){
   ...
   await new Promise<void>((resolve,reject)=>{ws.on('error',reject);ws.on('open',()=>ws.send(JSON.stringify({type:'join',room:'filter-test'})));ws.on('message',raw=>{const message=JSON.parse(String(raw));if(message.type==='welcome')resolve();if(message.type==='chat')received[i].push(message.text);});});
  }
  clients[0].send(JSON.stringify({type:'chat',text:'you BODOH!'}));
