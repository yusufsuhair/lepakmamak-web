...
for(const width of [390,1280])test(`map directory finds shops and hangouts at ${width}px`,async({page})=>{await page.setViewportSize({width,height:844});await page.goto('/');await page.getByRole('button',{name:"Jom, let's go"}).click();await page.locator('#auth-guest').click();await page.locator('#guest-name').fill('Tester');await page.getByRole('button',{name:'Enter as guest',exact:true}).click();await page.keyboard.press('m');await expect(page.locator('#city-map')).toBeVisible();await expect(page.locator('#city-map select')).toHaveCount(0);await expect(page.locator('#city-directory')).not.toContainText('LEGOLAND');await page.locator('#city-directory').getByRole('button',{name:'6 Kedai Aceh',exact:true}).click();await expect(page.locator('#map-place-info')).toContainText('Kedai Aceh');await expect(page.locator('#map-place-info')).toContainText('m away');await page.locator('#city-directory').getByRole('button',{name:'10 Dataran Santai',exact:true}).click();await expect(page.locator('#map-place-info')).toContainText('Dataran Santai');await page.screenshot({path:`test-results/map-places-${width}.png`});});
