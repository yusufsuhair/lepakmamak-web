...
test('voice never reaches distant listeners and follows movement',async()=>{
 ...
 const clients:WebSocket[]=[],received:any[][]=[[],[],[]];
 try{
 await expect.poll(async()=>{try{return (await fetch('http://127.0.0.1:8093/health')).ok;}catch{return false;}}).toBe(true);
 for(let i=0;i<3;i++){const ws=new WebSocket('ws://127.0.0.1:8093/ws');clients.push(ws);await new Promise<void>((resolve,reject)=>{ws.on('error',reject);ws.on('open',()=>ws.send(JSON.stringify({type:'join',room:'nearby'})));ws.on('message',raw=>{const m=JSON.parse(String(raw));if(m.type==='welcome')resolve();if(m.type==='voice-audio')received[i].push(m);});});ws.send(JSON.stringify({type:'voice-state',mic:i===0,speaker:true}));}
 ...
 await expect.poll(()=>received[1].length).toBe(1);expect(received[1][0].volume).toBeCloseTo(.5);expect(received[0]).toHaveLength(0);expect(received[2]).toHaveLength(0);
