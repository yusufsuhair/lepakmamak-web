...
for(const width of [1280,390])test(`find owner cars follows live fleet in 2D and 3D (${width}px)`,async({page})=>{
 ...
 await page.getByRole('button',{name:'3D',exact:true}).click();await expect(page.locator('#expanded-map')).toHaveAttribute('data-mode','3d');
 ...
 await page.getByRole('button',{name:'2D',exact:true}).click();await expect(page.locator('.car-finder-status')).toContainText('Daddy Fizal');
