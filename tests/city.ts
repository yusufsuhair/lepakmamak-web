...
// Routes a stand-in city, without entering it. The returned function moves the player: a later
// welcome restores a spot the same way a reconnect does, so specs can arrive and leave without
// walking. The stand-in answers the heartbeat; otherwise the client drops it as stale after
// ~13 s, which mutes ambience and would pass for a "pause". Ambience and background music also
// wait for a welcome, so a spec that asserts either needs this even when it never moves.
export async function standInCity(page:Page,x=-18,z=52){
...
// Enters as a guest at (x,z) against that stand-in city, and hands back its mover.
export async function enterAt(page:Page,x:number,z:number){
 const moveTo=await standInCity(page,x,z);
