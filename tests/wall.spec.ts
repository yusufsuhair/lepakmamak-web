...
test('Wall UI is responsive, renders mixed posts and opens a social profile',async({page})=>{
 ...
 await page.goto('/wall-harness');await page.evaluate(async()=>{const{setupWall}=await import('/src/wall.ts');(window as any).wall=setupWall('ws://wall.test/ws',()=>{},(id:string,name:string)=>{document.body.dataset.profile=`${id}|${name}`;});(window as any).wall.open();});
 ...
 await page.getByRole('button',{name:'Aina'}).click();await expect(page.locator('#social-wall')).not.toBeVisible();await expect(page.locator('body')).toHaveAttribute('data-profile','u2|Aina');await expect(page.locator('#wall-profile')).toHaveCount(0);
 await page.evaluate(()=>(window as any).wall.open());
