...
test('time scoring, hints and undo are authoritative',()=>{
 ...
 game.handle(ps,b,{type:'lukis-guess',text:answer});expect(events.some(e=>e.type==='lukis-correct')).toBeTruthy();expect(events.at(-1).game.scores.find((s:any)=>s.id==='a').score).toBe(40);
...
test('seating opens game cards and one selected game on mobile',async({page})=>{
 await page.setViewportSize({width:390,height:844});await page.goto('/');await page.evaluate(async()=>{const{setupTableSocial}=await import('/src/table-social.ts');const ui=setupTableSocial(()=>true,'test',()=>{});ui.state([{id:'meja-1',name:'Meja 1',capacity:3,occupants:[{id:'a',name:'A',chairId:'chair-1'}]}],'a',true);});
