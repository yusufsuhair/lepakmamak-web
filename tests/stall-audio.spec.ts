...
test('booth voice fades smoothly away from the nearest stall',async({page})=>{
  ...
  expect(result.near).toBe(0);expect(result.volumes[0]).toBe(.44);expect(result.volumes[result.full]).toBe(.44);expect(result.volumes[result.reach]).toBe(0);
  for(let distance=result.full+1;distance<=result.reach;distance++)expect(result.volumes[distance]).toBeLessThan(result.volumes[distance-1]);
});
...
test('booth voice loops after entry and obeys City sounds',async({page})=>{
  ...
  await expect.poll(async()=>(await state()).playing).toBe(true);await expect.poll(async()=>(await state()).gain).toBeGreaterThan(.001);
