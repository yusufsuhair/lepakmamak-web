...
test('invited player sits, treats a friend and exports a real session receipt',async({page})=>{
 ...
 friend=new WebSocket('ws://127.0.0.1:8088/ws');let friendId='';friend.on('message',raw=>{const m=JSON.parse(String(raw));if(m.players)peers=m.players;if(m.type==='welcome')friendId=m.id;});friend.on('open',()=>friend!.send(JSON.stringify({type:'join',room:'table-flow',guest:true,name:'Kawan'})));await expect.poll(()=>friendId).not.toBe('');
