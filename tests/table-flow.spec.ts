...
test('invited player sits, treats a friend and exports a real session receipt',async({page})=>{
 ...
 await page.locator('#open-tables').click();await expect(page.locator('#table-name-form')).toBeVisible();await page.locator('#table-name-input').fill('Geng Balik Lambat');await page.locator('#table-name-form button').click();await expect(page.locator('#table-name')).toHaveText('Geng Balik Lambat');
 ...
 await page.getByRole('button',{name:'🫓 Roti canai',exact:true}).click();await expect(page.locator('#mamak-order-status')).toContainText('Roti canai is served');await expect.poll(()=>sharedTables[0]?.occupants.some((p:any)=>p.order==='roti-canai')).toBe(true);
 await page.locator('#mamak-consume').click();await expect.poll(()=>sharedTables[0]?.occupants.every((p:any)=>!p.order)).toBe(true);
