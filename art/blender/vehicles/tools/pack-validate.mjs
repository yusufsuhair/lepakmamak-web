...
for (const item of manifest) {
 for (const [lod,ratio,error] of [['', '0.6','0.0015'],['-mid','0.24','0.006'],['-far','0.08','0.025']]) {
  ...
  execFileSync(process.execPath,[path.join(here,'node_modules/gltfpack/cli.js'),'-i',input,'-o',output,'-cc','-ce','ext','-kn','-km','-ke','-si',ratio,'-se',error,...(lod?['-sp']:[]),'-gt']);
  ...
  results.push({...item,file,lod:lod||'near',sourceBytes:item.bytes,bytes:bytes.length,triangles,sha256:createHash('sha256').update(bytes).digest('hex'),errors:0,warnings:0});
 }
}
