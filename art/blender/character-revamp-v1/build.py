...
random.seed(19)
OUT=os.path.abspath('art/blender/generated/character-revamp-v1')
...
white=mat('Eyes | warm ivory','#fff6e4',.24);iris=mat('Eyes | rich brown','#5b3520',.28);pupil=mat('Eyes | pupil','#100b09',.19)
...
def person(name,sk,shirt,pants,bob=False,covered=False):
 ...
 # Custom head shaping: narrower jaw, rounded cheeks, larger cranium.
 ob=uv('Face | sculpted soft jaw',(0,0,1.97),(1,1,1),sk,80,56)
 for v in ob.data.vertices:
  x,y,z=v.co;w=1 if z>-.25 else 1-.23*((-z-.25)/.75)
  v.co=(x*.427*w,y*.337, z*.458)
 for s in [-1,1]:
  uv('Ear',(s*.408,.012,1.943),(.069,.049,.105),sk)
  ear=mat(name+f' ear warmth {s}','#a66649' if sk==skin else '#bb7f59',.55,skin=True)
  uv('Ear | inner fold',(s*.449,-.013,1.947),(.02,.027,.053),ear,32,20)
  x=s*.162
  uv('Eye | sclera',(x,-.306,2.003),(.098,.033,.069),white)
  uv('Eye | amber iris',(x+s*.005,-.337,2.005),(.051,.014,.055),iris)
  uv('Eye | pupil',(x+s*.005,-.349,2.007),(.027,.008,.034),pupil)
  uv('Eye | catchlight',(x-.014,-.357,2.032),(.012,.004,.011),white,24,16)
  curve('Eye | upper eyelid',[(x-.095,-.323,2.013),(x-.055,-.345,2.067),(x+.03,-.35,2.076),(x+.095,-.321,2.021)],.009,sk)
  curve('Eye | fine lash line',[(x-.092,-.33,2.017),(x-.045,-.353,2.069),(x+.035,-.352,2.072),(x+.094,-.324,2.021)],.0035,brown if bob else hair)
  curve('Face | expressive brow',[(x-.083,-.285,2.132),(x-.02,-.31,2.158),(x+.077,-.283,2.141)],.012,brown if bob else hair)
 uv('Nose | soft bridge',(0,-.326,1.965),(.055,.063,.075),sk)
 uv('Nose | rounded tip',(0,-.38,1.927),(.061,.049,.044),sk)
 lip=mat(name+' | natural lip','#965342' if sk!=skin2 else '#b87560',.5,skin=True)
 curve('Mouth | soft smile',[(-.102,-.293,1.817),(-.055,-.322,1.795),(0,-.332,1.79),(.06,-.32,1.8),(.103,-.292,1.823)],.006,lip)
 curve('Mouth | lower lip',[(-.065,-.316,1.782),(0,-.329,1.771),(.061,-.314,1.785)],.009,sk)
 if covered:hijab()
...
bpy.ops.wm.save_as_mainfile(filepath=f'{OUT}/source/character-revamp-v1.blend')
...
cam.location=(.3,-8,2.95);cam.rotation_euler=(Vector((0,0,1.93))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=5.6;scene.render.resolution_x=1800;scene.render.resolution_y=700
