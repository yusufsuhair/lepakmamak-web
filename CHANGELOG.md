...
- Added Ah Meng, a house opponent for UNO Lepak. When somebody has waited six seconds alone in an UNO lobby and the table has a free chair, he sits down, readies up and plays; he leaves the moment nobody is left to play with, and never joins when two people are already there. He is an in-process client: his socket receives only what a browser would (his own hand), and every move goes through the same lobby and game handlers as a player's message, so `uno.mjs` is unchanged and there is nothing for him to cheat with. His name is "Ah Meng · AI", he takes no guest seat from the quota, and he appears in the room like anybody else. The first-steps guide now names UNO, because it is the game that can start in an empty city. UNO only for now.
- Replaced the six-step "Cara main" card that opened over the city on a first visit with a guided first minute. A new player is led to the nearest free mamak chair, then to the name above their table, one instruction at a time, with the button to tap lit up: Sit first, then the table's own name, which is the step the old card's note said most players never found. It returns on the next visit until they open a table's games or wave it off, as the card did until its box was ticked, and it uses the card's storage key so anyone who dismissed the card is not walked through it. "Ada lagi!" now waits until the first steps are done. The card stays in Settings as reference, without its checkbox.
...
- Fixed the KLCC lift cabin being swallowed by the city-wide static batch, which left the rider in an empty shaft.
